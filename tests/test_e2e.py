...
def test_full_summarize_chain(sample_chat_session, tmp_path):
    ...
    input_file.write_text(sample_chat_session.model_dump_json(ensure_ascii=False, indent=2), encoding="utf-8")
...
def test_llm_error_preserves_raw_data(mock_device, sample_messages, tmp_path):
    ...
    assert result.exit_code == 1
    raw_files = [f for f in Path(tmp_path).glob("异常群_*.json") if not f.name.endswith("_summary.json")]
...
def test_summarize_markdown_format(sample_chat_session, tmp_path):
    ...
    with patch("wechat_summary.cli.ChatSummarizer") as mock_summarizer_cls:
        mock_summarizer = mock_summarizer_cls.return_value
...
def test_summarize_empty_chat(tmp_path):
    ...
    input_file.write_text(empty_session.model_dump_json(ensure_ascii=False, indent=2), encoding="utf-8")
